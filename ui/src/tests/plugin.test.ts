...
import {LivechatTest, newTest, newPluginDir} from './setup';
...
beforeAll(async () => {
    const LivechatPluginDir = pluginSupported
        ? await newPluginDir(['github.com/Livechat/server/v2/plugin/example/echo'])
        : '';
    Livechat = await newTest(LivechatPluginDir);
    page = Livechat.page;
});
            ...
            it('has plugin info', async () => {
                await inDetailPage(1, async () => {
                    expect(await pluginInfo('module-path')).toBe(
                        'github.com/Livechat/server/v2/plugin/example/echo'
                    );
                });
            });
