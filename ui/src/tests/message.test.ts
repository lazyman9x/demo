...
import {newTest, LivechatTest} from './setup';
...
let page: Page;
let Livechat: LivechatTest;
beforeAll(async () => {
    Livechat = await newTest();
    page = Livechat.page;
});

afterAll(async () => await Livechat.close());
...
describe('Messages', () => {
    ...
    const createApp = (name: string) => {
        return axios
            .post<IApplication>(`${Livechat.url}/application`, {name}, axiosAuth)
    ...
    const backup3 = m('Backup done', 'Livechat Backup finished (0.1MB).');

    const createMessage = (msg: Partial<IMessage>, token: string) => {
        return axios.post<IMessage>(`${Livechat.url}/message`, msg, {
            headers: {'X-Livechat-Key': token},
        });
    };
